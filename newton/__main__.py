...
parser.add_argument('-i', '--init', help='Initialize profile info', action="store_true")
...
parser.add_argument('--ui', help='Start the QT UI', action="store_true")
...
if args.init:
    ...
    wait(ui.master(loop), loop=loop)
elif args.backup:
    wait(backup_resources(), loop=loop)
elif args.follow:
    wait(follow(args.follow), loop=loop)
elif args.message:
    wait(post_newt(args.message, loop), loop=loop)
elif args.reply:
    wait(post_reply(args.reply[0], args.reply[1]), loop=loop)
elif args.repost:
    wait(post_renewt(args.repost[0]), loop=loop)
elif args.like:
